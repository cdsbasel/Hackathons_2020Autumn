...
5. Create a `tibble` using `tibble` that has two columns: a column called `book` containing a self-defined character vector of book names and a column called `text` containing the `texts` vector of books. Call the tibble `text_tbl`. When you subsequently print it it should like the output shown below. 
